...
class EDD_Articlemag_Theme_Updater_Admin {
	...
	function articlemag_license_action( $old_value ) {		
		...
		if ( $license_action ) {
			if ( $license != $old_value['articlemag_license'] || $old_value['license_action'] == 0 ) {				
	...
	function get_license_page_url() {
		return admin_url( 'admin.php?page=articlemag_theme_options' );
	}
	...
	function activate_license( $license ) {
		...
		if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
			...
			if ( false === $license_data->success ) {
				...
				if ( ! empty( $message ) ) {
					...
					wp_send_json_success( array( 'success' => true, 'notice' => $message, 'errors' => '','page_reload'=> true) );
					return true;
		...
		wp_send_json_success( array( 'success' => true, 'notice' => __( 'Activate License Key', 'articlemag'), 'errors' => '','page_reload'=> true) );
		return true;
	...
	function deactivate_license( $license ) {
		...
		wp_send_json_success( array( 'success' => true, 'notice' => __( 'Deactivate License Key.', 'articlemag' ), 'errors' => '','page_reload'=> true) );
		return true;
