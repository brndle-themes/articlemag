<?php
/**
 * Articlemag Profile Completion Widget.
 */
 ...
 * @subpackage Widgets
 */
class BP_Articlemag_Profile_Completion_Widget extends WP_Widget {
    ...
    function __construct() {
        ...
        $widget_ops = array(
            ...
            'description' => __('Show Logged in user Profile Completion Progress.', 'articlemag'),
        );

        /* Set up the widget. */
        parent::__construct(
                false, __('(BuddyPress) Profile Completion', 'articlemag'), $widget_ops
        );
    }
    ...
    function widget($args, $instance) {
        ...
        $progress_label = sprintf(__('%s Complete', 'articlemag'), $user_progress['completion_percentage'] . '%');
        ?>
                ...
                <div class="wb-bp-progress-text">
                    <div class="wb-bp-user-avatar-wrap">
                        ...
                        <div class="wb-bp-progress-label">
                            ...
                            <span><?php echo esc_html__('Complete', 'articlemag'); ?></span>
                        </div>
                    </div>
                </div>
    ...
    function get_user_profile_progress($group_ids, $photo_types) {
        ...
        if (!$is_profile_photo_disabled && in_array('profile_photo', $photo_types)) {
            ...
            $progress_details['photo_type']['profile_photo'] = array(
                ...
                'name' => __('Profile Photo', 'articlemag'),
            );
        }
        ...
        if (!$is_cover_photo_disabled && in_array('cover_photo', $photo_types)) {
            ...
            $progress_details['photo_type']['cover_photo'] = array(
                ...
                'name' => __('Cover Photo', 'articlemag'),
            );
        }
        ...
        $progress_details['groups'][] = array(
            'group_name' => __('Profile Fields', 'articlemag'),
        ...
        return apply_filters('BP_articlemag_user_progress', $progress_details);
    }
    ...
    function get_user_profile_progress_formatted($user_progress_arr) {
         ...
         */
        return apply_filters('BP_articlemag_user_progress_formatted', $user_prgress_formatted);
    }
    ...
    function update($new_instance, $old_instance) {
        return apply_filters('BP_articlemag_profile_completion_form_update', $new_instance);
    }
    ...
    function form($instance) {
        ...
        $instance = wp_parse_args(
            ...
            'title' => __('Complete Your Profile', 'articlemag'),
        ...
        if (!$is_profile_photo_disabled) {
            $photos_enabled_arr['profile_photo'] = __('Profile Photo', 'articlemag');
        }
        if (!$is_cover_photo_disabled) {
            $photos_enabled_arr['cover_photo'] = __('Cover Photo', 'articlemag');
        }

        /* Widget Form HTML */
        ?>
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php esc_html_e('Title:', 'articlemag'); ?></label>
            <input class="widefat" id="<?php echo esc_attr($this->get_field_id('title')); ?>" name="<?php echo esc_attr($this->get_field_name('title')); ?>" type="text" value="<?php echo esc_attr($instance['title']); ?>"/>
        </p>    
        <p>
            <label><?php esc_html_e('Profile field sets:', 'articlemag'); ?></label>
        ...
        <?php if (!empty($photos_enabled_arr)) : ?>
            <p>
                <label><?php esc_html_e('Profile photos:', 'articlemag'); ?></label>
            <ul>
        ...
        <p>
            <label>
                ...
                <?php echo __('Hide widget once progress hits 100%', 'articlemag'); ?>
            </label>

        </p>
        <p>
            <small>
                <?php esc_html_e('Note: This widget is only displayed if a member is logged in.', 'articlemag'); ?>
            </small>
        </p>
...
add_action(
    ...
    if (class_exists('BuddyPress')) {
        register_widget('BP_Articlemag_Profile_Completion_Widget');
    }
}
);
